# Copyright 2021 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# output "video_splitter_url" {
#   value       = google_cloud_run_v2_service.dlp_runner.uri
#   description = "video Splitter function url"
# }

# output "video_merger_url" {
#   value       = google_cloud_run_v2_service.video_merger.uri
#   description = "video Splitter function url"
# }

# output "dlp_runner_url" {
#   value       = google_cloud_run_v2_service.dlp_runner.uri
#   description = "video Splitter function url"
# }

# output "findings_writer_url" {
#   value       = google_cloud_run_v2_service.findings_writer.uri
#   description = "video Splitter function url"
# }

output "video_input_bucket" {
  value       = google_storage_bucket.video_input_bucket.url
  description = "Bucket that will be used to drop/land your raw pfd files for redaction."
}

output "workflow_working_bucket" {
  value       = google_storage_bucket.working_bucket.url
  description = "Bucket that will be use for temp files, this will automatically be emptied/scrapped every day."
}

output "video_output_bucket" {
  value       = google_storage_bucket.video_output_bucket.url
  description = "Bucket that will be used to output your processed pfd files after redaction."
}

output "function_uri" {
  value = google_cloudfunctions2_function.video_intelligence_function.service_config[0].uri
}

# output "video_bq_connection" {
#   value       = google_bigquery_connection.connection.connection_id
#   description = "BQ connection id"
# }

# output "video_bq_dataset" {
#   value       = google_bigquery_dataset.video_analytics.dataset_id
#   description = "BQ dataset id"
# }

# output "workflow_name" {
#   value       = google_workflows_workflow.video_intelligence.name
#   description = "Workflow that orchestrated the redaction of a single video file"
# }
